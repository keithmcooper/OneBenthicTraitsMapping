...
## Reduce size of raster using aggregate function (e.g. aggregate from 40x40 resolution to 120x120 (factor = 3))
...
## Check there are only 12 cluster groups
unique(sdf$FCluster)
...
modfc=ggplot(data=sdf, aes(y=Latitude, x=Longitude)) +
  ...
  #scale_fill_manual(values = c('#4575b4', '#00E600', '#91bfdb', '#e0f3f8','#fee090','#fc8d59'),name="Cluster")+# RESPONSE TRAITS
  scale_fill_manual(values = c('#5ab4ac', '#d8b365', '#c7eae5', '#8c510a','#f6e8c3', '#FF0000'),name="Cluster")+# EFFECTS TRAITS
...
## Load raster data
bathy <- raster("DATA/Rasters/Final/bathy3.tif")
#bathy<- raster("C:/Users/kmc00/OneDrive - CEFAS/R_PROJECTS/RNSPBiotopeMap/DATA/bathy3.tif")
# CHOOSE EITHER
#pr = raster('DATA/ResponseTraitsCluster.tif')# RESPONSE TRAITS OR
pr = raster('DATA/EffectsTraitsCluster.tif') # EFFECTS TRAITS

## Produce simplified version of raster layers
bathy.agg <- aggregate(bathy, fact=8,fun = modal)
pr.agg <- aggregate(pr, fact=8,fun = modal)

## Convert bathy to matrix
elmat = raster_to_matrix(bathy.agg)

## 3D bathy plot in two dimensions
elmat%>% sphere_shade(texture="desert")%>%plot_map()

## 3D bathy plot in three dimensions
elmat%>% sphere_shade(texture="desert")%>%
  plot_3d(elmat, zscale = 10, fov = 0, theta = 135, zoom = 0.75, phi = 45, windowsize = c(1000, 800))
Sys.sleep(0.2)
render_snapshot()

## 3D bathy plot in three dimensions with water
elmat%>% sphere_shade(texture="desert")%>%
  plot_3d(elmat, zscale = 10, fov = 0, theta = 135, zoom = 0.75, phi = 45, windowsize = c(1000, 800),water = TRUE, waterdepth = -40, wateralpha = 0.5, watercolor = "lightblue",waterlinecolor = "white", waterlinealpha = 0.5)
#_______________________________________________________________________________
#### FINAL 3D PLOT ####
...
## Bring in traits raster
# CHOOSE EITHER 
pr = raster('DATA/ResponseTraitsCluster.tif') # RESPONSE TRAITS OR
#pr = raster('DATA/EffectsTraitsCluster.tif') # EFFECTS TRAITS
pr.agg <- aggregate(pr, fact=8,fun = modal)# reduce the size of the raster
#plot(pr.agg)

## Convert raster to matrix for use with rayshader 
elmat = raster_to_matrix(bathy.agg)
...
## Set working directory
# CHOOSE EITHER
#setwd('C:/Users/kmc00/OneDrive - CEFAS/R_PROJECTS/TraitsMapping/OUTPUTS/RESPONSE_TRAITS/')# OR
setwd('C:/Users/kmc00/OneDrive - CEFAS/R_PROJECTS/TraitsMapping/OUTPUTS/EFFECTS_TRAITS/')
...
## make a movie using https://gifmaker.me/ Use Animation seed: 125
#_______________________________________________________________________________
#### DRAIN THE SEA #####
...
for (i in seq_len(n_frames)) {
  ...
  elmat %>%
    ...
    plot_3d(elmat, solid = TRUE, shadow = TRUE, zscale = zscale, 
            water = TRUE, watercolor = "imhof3", wateralpha = 0.94, 
...
## Build gif
#https://gifmaker.me/ # use this site to create gif as code below not reliable
#_______________________________________________________________________________
